...
fun findValidGames(games: List<Game>): Int {
    ...
    games.forEach gameLoop@ { game ->
    	game.rounds.forEach { round ->
        	...
        	round.forEach { pair ->
                when (pair.second) {
                    "green" -> greenCount += pair.first
                    "red" -> redCount += pair.first
                    "blue" -> blueCount += pair.first
                }
               	if (greenCount > GREEN_MAX || redCount > RED_MAX || blueCount > BLUE_MAX) return@gameLoop
            }
        }
        sum += game.id
    }
...
fun findValidGamesHardMode(games: List<Game>): Int {
    var power = 0
    games.forEach { game ->
        var greenMin = 0
        var redMin = 0
    	...
    	game.rounds.forEach { round ->  
        	round.forEach { pair ->
                ...
                var blueCurrent = 0
                when (pair.second) {
                    "green" -> greenCurrent += pair.first
                    "red" -> redCurrent += pair.first
                    "blue" -> blueCurrent += pair.first
                }
               	greenMin = if (greenMin < greenCurrent) greenCurrent else greenMin
                redMin = if (redMin < redCurrent) redCurrent else redMin
                blueMin = if (blueMin < blueCurrent) blueCurrent else blueMin
            }
        }
        power += (greenMin*redMin*blueMin)
    }
    return power
}
...
data class Game(
	val id: Int,
    val rounds: List<List<Pair<Int, String>>>
)