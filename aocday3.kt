...
fun getEndIndex(line: String, index: Int): Int {
    var i = index
...
fun hasAdjacentSymbols(grid: List<String>, lineIndex: Int, startIndex: Int, endIndex: Int): Boolean {
    // Sweep left.
    if (startIndex > 0 && isSymbol(grid[lineIndex][startIndex - 1])) return true
    // Sweep right.
    if (endIndex < grid[lineIndex].length - 1 && isSymbol(grid[lineIndex][endIndex + 1])) return true   
    // Sweep top. 
    val topLineIndex = lineIndex - 1
    if (topLineIndex >= 0 && sweepLine(grid[topLineIndex], startIndex - 1, endIndex + 1)) return true
    // Sweep bottom.
    val bottomLineIndex = lineIndex + 1  
    if (bottomLineIndex < grid.size && sweepLine(grid[bottomLineIndex], startIndex - 1, endIndex + 1)) return true    
    return false
}

fun sweepLine(line: String, startIndex: Int, endIndex: Int): Boolean {
    for (i in startIndex..endIndex) if (i >= 0 && i < line.length && isSymbol(line[i])) return true
    return false
}

fun addByBounds(line: String, startIndex: Int, endIndex: Int): Int {
    var sum = ""
...
// Part 2.
fun engineSchematicSolverHardMode(grid: List<String>): Int {
    var sum = 0
    grid.forEachIndexed { lineIndex, line ->
        var strIndex = 0
        while (strIndex < line.length) {
        	if (line[strIndex] == '*') {
                sum += getGearRatio(grid, lineIndex, strIndex)
            }
            strIndex++
        }
    }
...
fun getGearRatio(grid: List<String>, lineIndex: Int, strIndex: Int): Int {
    var numCount = 0
    var product = 1
    var startIndex = strIndex - 1
    // Sweep left.
    if (startIndex > 0 && grid[lineIndex][startIndex].isDigit()) {
        product *= getCompleteDigit(grid[lineIndex], startIndex).first
        numCount++
    } 
    // Sweep right.
    startIndex = strIndex + 1
    if (startIndex < grid[lineIndex].length && grid[lineIndex][startIndex].isDigit()) {
        product *= getCompleteDigit(grid[lineIndex], startIndex).first
        numCount++
    }
    // Sweep top.
    startIndex = strIndex - 1
    var topIndex = lineIndex - 1
    if (topIndex >= 0) {
        while(startIndex <= strIndex + 1) {
            if (startIndex < grid[topIndex].length && startIndex >= 0 && grid[topIndex][startIndex].isDigit()) {
                val results = getCompleteDigit(grid[topIndex], startIndex)
                product *= results.first
                startIndex = results.second
                numCount++
            }
            startIndex++
        }
    ...
    // Sweep bottom.
    startIndex = strIndex - 1
    var bottomIndex = lineIndex + 1
    if (bottomIndex >= 0) {
        while(startIndex <= strIndex + 1) {
            if (startIndex < grid[bottomIndex].length && startIndex >= 0 && grid[bottomIndex][startIndex].isDigit()) {
                val results = getCompleteDigit(grid[bottomIndex], startIndex)
                product *= results.first
                startIndex = results.second
                numCount++
            }
    ...
    return if (numCount != 2) 0 else product 
}

fun getCompleteDigit(line: String, index: Int): Pair<Int, Int> {
    var str = "${line[index]}"
    var left = index - 1
    var right = index + 1
